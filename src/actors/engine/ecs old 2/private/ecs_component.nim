...
template impl_storage(T: typedesc) {.used.} =
  ...
  storages.add(storage)
  storage.compType = $T
  #storage.actiondel = cast[byte](storage.comps.addr
 ...
 # echo T.sizeof
  ...
  proc remove(t: typedesc[T], self: ent) {.inline, discardable.} = 
   ...
   # let entity = addr ents_meta[self.id]
    ...
    let meta = ents_meta[self.id].addr
    let groups = storage.groups
    for group in groups:
      let grouped = isGrouped(meta, group)
      let isvalid = isValidForGroup(self.id, group)
   
      if not grouped and isvalid:
        var cstorages = newSeq[CompStorageBase](meta.signature.len)

        for i in 0..meta.signature.high:
          cstorages[i] = storages[meta.signature[i].int]
       
        cstorages.sort(sortStorages)
        group.entities = cstorages[0].entities.addr
      elif grouped and not isvalid:
        group.entities = nil
  ...
  storage.istorage = IStorage(destroy: proc(self: ent) = T.remove(self))
