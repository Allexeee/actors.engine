...
template impl_entity(layerID: uint32): ent =
  let ecs = layers[layerID].addr
  if ents_stash.len > 0:
    result = ents_stash[0]
...
var action_check_error_remove_component* : EcsErrorTypedAction = impl_ecs_check_error_remove_component
var action_check_error_release_empty*    : EcsErrorAction      = impl_ecs_check_error_release_empty
...
proc impl_ecs_check_error_remove_component(self: ent, t: typedesc) =
    ...
    if t.Id notin entities[self.id].signature:
      echo &"You are trying to remove a {arg1} that is not attached to entity with id {arg2}"
...
proc impl_ecs_check_error_release_empty(self: ent) =
    ...
    if entities[self.id].signature.card == 0:
      echo &"You are trying to release an empty entity with id {arg1}. Entities without any components are released automatically."
...
#@errors
template check_error_remove_component(self: ent, t: typedesc): untyped =
  when defined(debug):
    action_check_error_remove_component(self, t)

template check_error_release_empty(self: ent): untyped =
  when defined(debug):   
    action_check_error_release_empty(self)